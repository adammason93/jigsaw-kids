...
# supabase secrets set STORYBOOK_GPTIMAGE_QUALITY=low           # low | medium | high | auto — code default is medium (sharper); use low if 546 timeouts / need speed
# supabase secrets set STORYBOOK_GPTIMAGE_INPUT_FIDELITY=low    # edits: low | high — code default high (sharper, tighter to anchor); use low for speed
