...
# supabase secrets set STORYBOOK_GPTIMAGE_INPUT_FIDELITY=high   # edits only: high | low (default high for cast lock)
...
# supabase secrets set STORYBOOK_GPTIMAGE_CHUNK_WAIT_MS=20000   # cooldown between chunks (default 20000ms; raise to 35000+ if you still see 429s on tier 1)
```
...
- **GPT Image (`STORYBOOK_IMAGE_MODE=gptimage`)** — **strict, no fallback**: When this mode is on, the function will ONLY use the configured GPT Image model (**default `gpt-image-1.5`** if `STORYBOOK_GPTIMAGE_MODEL` is unset). There is **no silent fallback** to Fal or DALL·E — if the anchor or any spread edit fails, the function returns `error: "gpt_image_failed"` with the OpenAI detail in `detail` and `imageMode: "gptimage"` so the client knows exactly which engine failed. Anchor uses **`/v1/images/generations`**; the 6 spread edits use **`/v1/images/edits`** with the anchor PNG as reference. Requests set **`moderation: low`** by default (override with `STORYBOOK_GPTIMAGE_MODERATION=auto` if you prefer OpenAI’s stricter filter). **`output_format: png`**, **`stream: false`**, and **HTTP 400 fallbacks** (minimal generation payload; edits retry without `quality`) reduce empty responses and parameter rejections. Output is decoded from base64 (or fetched if a URL is ever returned) and uploaded to the public **`storybook_images`** Supabase Storage bucket (service role key auto-injected). The spread edits run in **chunked parallel** — by default 4 edits at once (anchor + 4 = 5/min, the tier-1 ceiling), then a ~20s cooldown so the anchor’s request leaves the per-minute window, then the remaining edits in parallel. The text-based "visual lock" vision call is **skipped** for this pipeline because the anchor PNG itself is already attached to every spread edit — a second pass would just burn ~10s for no identity benefit. The full pipeline targets ~95–110s wall-clock to stay safely under the Supabase/Cloudflare 150s timeout (HTTP 504/546) while respecting OpenAI’s rate cap. A built-in **429 retry-after backoff** mops up any short bursts. Tune via `STORYBOOK_GPTIMAGE_CHUNK_SIZE` and `STORYBOOK_GPTIMAGE_CHUNK_WAIT_MS`. Errors usually come from missing bucket (run the migration), prompt moderation, or hitting tier-1 rate limits (raise cooldown to 35000ms or upgrade OpenAI tier) — check function logs.
