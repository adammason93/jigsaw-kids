...
# supabase secrets set STORYBOOK_GPTIMAGE_QUALITY=low           # low | medium | high | auto — default low (faster, fits ~150s edge limit); use medium for nicer art
# supabase secrets set STORYBOOK_GPTIMAGE_INPUT_FIDELITY=low    # edits: low | high — default low (faster); high = stricter cast match, slower
...
- **HTTP 546 / 504 / “ran out of time” in the storybook app**: The Edge Function has a ~150s wall-clock limit. GPT Image does **1 anchor + 6 edits** (with a **pause between edit batches** for OpenAI tier-1 rate limits). Defaults now favour **fast** settings (`quality: low`, `input_fidelity: low`, **12s** between batches). If it still times out, set `STORYBOOK_GPTIMAGE_CHUNK_WAIT_MS` lower only if your OpenAI tier allows **more images/minute**; otherwise upgrade OpenAI tier. For prettier (slower) output, set `STORYBOOK_GPTIMAGE_QUALITY=medium` and/or `STORYBOOK_GPTIMAGE_INPUT_FIDELITY=high` and ensure your account finishes within the edge limit.
